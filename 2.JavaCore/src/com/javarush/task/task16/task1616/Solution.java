...
public class Solution {
    ...
    public static class Stopwatch extends Thread {
        // поле для подсчета секунд ожидания
        private int seconds;
        ...
        public void run() {
            ...
            // на объект вызвавшей ее нити
            Thread current = Thread.currentThread();
            while (!current.isInterrupted()) {
                try {
                    // итерирование секунд
                    seconds++;
                    // тормозим поток на секунду
                    Thread.sleep(1000);
                } catch (InterruptedException e) {
                    // завершение будет через catch
                    // поэтому тут вывожу результат на экран
                    // после чего обяязательный ритерн
