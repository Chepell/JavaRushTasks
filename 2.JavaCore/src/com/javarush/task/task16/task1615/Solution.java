...
public class Solution {
    // создание объекта взлетной полосы
    // с модификатором volatile который говорит что переменная будет меняться
    // не кэшировать ее, не помещать в кэш потока
    ...
    // метод наследуется от Thread
    public static class Plane extends Thread {

        // для метода наследника от Thread
        // обязательно переопределять конструктор
        // имя для потока обязательное
        public Plane(String name) {
            // передаю аргумент имени в родительский класс
            super(name);
            // и запускаю нить из конструктора при создани объекта
            start();
        }

        // реализация метода потока
        public void run() {
            // переменная фиксирующая взлет
            boolean isAlreadyTakenOff = false;
            // цикл работает пока переменная взлета не поменяется на true
    ...
    public static class Runway { //взлетная полоса
        ...
        // метод проверяет свободна ли взлетная полоса
        // если свободна размеет в ней поток и возвращает true
