...
public class Solution {
    ...
    // Класс добавления записок
        ...
        public void addNote(int index, String note) {
            ...
            // т.к. список это Object, то у него есть мюьтекс (монитор)
            // и неатомарные операторы можно окружать блоком synchronized и указывать аргументом объект,
            // что бы избежать ошибок при многопоточной работе если первый поток занял этот мьютекс,
            // то второй не полезет в него, пока первый не освободит мьютекс
            synchronized (notes) {
