...
/* 
Посчитаем
1. Сделай так, чтобы результат успел посчитаться для всех элементов
    массива values НЕ используя Thread.sleep в методе main(String[] args)
...
6. Для каждой нити, в методе main(String[] args) класса
    Solution используй метод, который ожидает завершение нити.
*/
...
public class Solution {
    ...
    // инициализация поле при зарузке класса
    public static Integer count = 0;
    public static int[] values = new int[105];

    // после инициализации полей выполнение статик-блока
    static {
        for (int i = 0; i < 105; i++) {
            values[i] = 0;
        }
