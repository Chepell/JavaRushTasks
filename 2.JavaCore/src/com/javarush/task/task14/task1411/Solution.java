...
2. Метод main должен прекращать считывать строки с клавиатуры, как только
    введенная строка не совпадает с одной из ожидаемых(user, loser, coder, proger).
3. Для каждой корректной(user, loser, coder, proger) введенной
    строки должен быть вызван метод doWork с соответствующим объектом класса Person в качестве параметра.
...
public class Solution {
    public static void main(String[] args) throws Exception {
        ...
        Person person = null;
        // начальный ввод ключа
        String key = reader.readLine().toLowerCase();

        // захожу в цикл, если ключ соответствует одному из вариантов
        while (key.equals("user") || key.equals("loser") || key.equals("coder") || key.equals("proger"))
        {
            // по ключу создаю объект подходящего класса
            if (key.equals("user")) {
            ...
            // вызываем doWork
            doWork(person);
            // ввод новго ключа перед повтором цикла
            key = reader.readLine().toLowerCase();
        }
    }

    public static void doWork(Person person) {
        // проверка, что объект является экземпляром конкретного класса
        if (person instanceof Person.User) {
            //downcast - нисходящее преобразование от интерфейса к классу
