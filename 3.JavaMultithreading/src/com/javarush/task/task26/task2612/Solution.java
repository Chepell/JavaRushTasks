...
public class Solution {
    ...
    public void someMethod() {
        // пробую захватить лок елси он свободен
        // метод возвращает true в случае успеха
        if (lock.tryLock()) {
            // если получилось и поток был свободен, то вызываю метод
            try {
                ifLockIsFree();
            } catch (Exception e) {
                e.printStackTrace();
            } finally {
                // вконце освобождаю лок в болке finally что бы в случае исключения
                // в блоке try лок в любом случае освободился
                lock.unlock();
            }
            // если же поток захватить не удалось, то понятно что он занят
        } else {
            ifLockIsBusy();
        }
    }
